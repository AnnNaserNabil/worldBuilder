/**
 * ECONOMY_ORACLE Agent
 * Generates economy data and creates economy dashboard component
 */
...
const resourceTypes = [
  { type: 'material', name: 'Iron Ore', baseValue: 10, rarity: 'common' },
  { type: 'material', name: 'Silver', baseValue: 25, rarity: 'uncommon' },
  { type: 'material', name: 'Gold', baseValue: 100, rarity: 'rare' },
  { type: 'material', name: 'Obsidian', baseValue: 15, rarity: 'common' },
  { type: 'magical', name: 'Void Crystals', baseValue: 200, rarity: 'legendary' },
  { type: 'magical', name: 'Shadow Essence', baseValue: 75, rarity: 'rare' },
  { type: 'magical', name: 'Mana Shards', baseValue: 50, rarity: 'uncommon' },
  { type: 'agricultural', name: 'Grain', baseValue: 5, rarity: 'common' },
  { type: 'agricultural', name: 'Wine', baseValue: 20, rarity: 'common' },
  { type: 'agricultural', name: 'Spices', baseValue: 35, rarity: 'uncommon' },
  { type: 'crafted', name: 'Steel Weapons', baseValue: 80, rarity: 'uncommon' },
  { type: 'crafted', name: 'Enchanted Armor', baseValue: 150, rarity: 'rare' },
  { type: 'crafted', name: 'Potions', baseValue: 30, rarity: 'common' },
  { type: 'crafted', name: 'Jewelry', baseValue: 60, rarity: 'uncommon' }
];

const cityNames = ['Shadowhaven', 'Ironhold', 'Nightspire', 'Bogwatch', 'Frostgate', 'Ravenspire', 'Blackhold', 'Voidmere', 'Emberfall', 'Mistral'];

function generateEconomyData(worldBible) {
  console.log('ECONOMY_ORACLE: Generating economy data...');
  
  const regions = worldBible?.geography?.regions || [];
  const regionNames = regions.length > 0 ? regions.map(r => r.name) : ['The Shadowed Vale', 'Iron Peaks', 'Bloodmarsh', 'Voidwood', 'Frostbite Tundra'];

  // Generate resources with region assignments
  const resources = resourceTypes.map(res => ({
    id: generateId('res'),
    name: res.name,
    type: res.type,
    description: `A ${res.rarity} ${res.type} resource ${randomElement(['highly sought after across the realm', 'essential for crafting', 'prized by collectors', 'controlled by powerful factions', 'traded along dangerous routes'])}.`,
    baseValue: res.baseValue,
    regions: [randomElement(regionNames), randomElement(regionNames)].filter((v, i, a) => a.indexOf(v) === i),
    rarity: res.rarity
  }));

  // Generate cities with production/consumption
  const cities = cityNames.map((name, index) => {
    const producedResources = resources
      .filter(() => Math.random() > 0.6)
      .slice(0, 3)
      .map(r => r.name);
    
    const consumedResources = resources
      .filter(r => !producedResources.includes(r.name))
    ...
    return {
      id: generateId('city'),
      name,
      production: producedResources.length > 0 ? producedResources : [randomElement(resources).name],
      consumption: consumedResources.length > 0 ? consumedResources : [randomElement(resources).name, randomElement(resources).name],
      wealth: randomRange(1000, 100000),
      population: randomRange(1000, 50000)
    };
  });

  // Generate trade routes
  const tradeRoutes = [];
  const numRoutes = randomRange(8, 15);
  
  for (let i = 0; i < numRoutes; i++) {
    const from = randomElement(cities);
    let to = randomElement(cities);
    while (to.id === from.id) {
      to = randomElement(cities);
    }

    const goods = from.production.slice(0, 2);
    ...
    tradeRoutes.push({
      ...
      toName: to.name,
      goods,
      volume: randomRange(100, 5000),
      danger: randomRange(1, 10),
      description: `A ${randomElement(['well-traveled', 'dangerous', 'ancient', 'secret', 'heavily guarded'])} route connecting ${from.name} and ${to.name}. ${randomElement(['Bandits lurk in the shadows.', 'The road is maintained by merchant guilds.', 'Caravans travel weekly.', 'Tolls are collected at checkpoints.'])}`
    });
  }

  // Generate market data
  const markets = resources.map(res => ({
    resource: res.name,
    basePrice: res.baseValue,
    volatility: parseFloat((Math.random() * 0.3 + 0.1).toFixed(2)),
    trend: randomElement(['rising', 'stable', 'falling'])
  }));

  return { resources, cities, tradeRoutes, markets };
}

function generateEconomyDashboardComponent() {
  console.log('ECONOMY_ORACLE: Generating economy dashboard component...');
...
const economyData = generateEconomyData(worldBible);

// Write economy.json
writeFileSync(
  join(process.cwd(), 'data', 'economy.json'),
  JSON.stringify(economyData, null, 2)
);
console.log('ECONOMY_ORACLE: Written economy.json');

// Write React component
writeFileSync(
  join(process.cwd(), 'src', 'components', 'EconomyDashboard.tsx'),
  generateEconomyDashboardComponent()
);
console.log('ECONOMY_ORACLE: Written EconomyDashboard.tsx');

// Create done marker
writeFileSync(join(process.cwd(), 'data', '.economy_oracle.done'), new Date().toISOString());
console.log('ECONOMY_ORACLE: Economy generation complete!');
